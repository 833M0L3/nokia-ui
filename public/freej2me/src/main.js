...
function setListeners() {
    ...
    window.addEventListener('message', (e) => {
        if (e.data && (e.data.type === 'j2me-key' || e.data.type === 'CHEERP_KEY_EVENT')) {
            const isDown = e.data.eventType === 'keydown' || e.data.isDown === true;
            let code = e.data.code || e.data.key;
            if (!code) return;

            // Map aliases to standard codeMap keys
            const keyAliasMap = {
                '1': 'Digit1', '2': 'Digit2', '3': 'Digit3',
                '4': 'Digit4', '5': 'Digit5', '6': 'Digit6',
                '7': 'Digit7', '8': 'Digit8', '9': 'Digit9',
                '0': 'Digit0', '*': 'NumpadAsterisk', '#': 'NumpadDivide',
                'SoftLeft': 'F1', 'Soft1': 'F1', 'SoftRight': 'F2', 'Soft2': 'F2',
                'UP': 'ArrowUp', 'DOWN': 'ArrowDown', 'LEFT': 'ArrowLeft', 'RIGHT': 'ArrowRight',
                'OK': 'Enter', 'SELECT': 'Enter'
            };
...
async function init() {
    ...
    if (sp.get('app')) {
        ...
        let jarParam = sp.get('url') || sp.get('game') || sp.get('jar') || "game.jar";
        let jarPath;
        if (jarParam.startsWith('blob:') || jarParam.startsWith('http://') || jarParam.startsWith('https://')) {
            jarPath = jarParam;
        } else if (jarParam.startsWith('/app/')) {
            jarPath = jarParam;
        } else if (jarParam.startsWith('/freej2me/')) {
            jarPath = '/app' + jarParam;
        } else if (jarParam.startsWith('/')) {
            jarPath = cheerpjWebRoot + jarParam;
        } else if (jarParam.startsWith('games/')) {
            jarPath = cheerpjWebRoot + "/" + jarParam;
        } else {
            jarPath = cheerpjWebRoot + "/games/jar/" + jarParam;
        }
        args = ['jar', jarPath];
    }
